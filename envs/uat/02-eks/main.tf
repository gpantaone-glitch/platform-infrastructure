...
resource "aws_eks_access_policy_association" "uat_admin_policy" {
  cluster_name  = module.eks.cluster_name
  principal_arn = aws_eks_access_entry.uat_admin.principal_arn
...
resource "aws_eks_access_entry" "terraform_deployer" {
  cluster_name  = module.eks.cluster_name
  principal_arn = "arn:aws:iam::997208471891:role/terraform-deployer-role"
  type          = "STANDARD"
}
